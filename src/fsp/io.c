...
void FSPFlags2BitField(FSPState* fsp_state, uint32_t* trigger_field, uint32_t* event_field)
{
  ...
  efield |= ((fsp_state->proc_flags.hwm.hw_multiplicity & 0x1)           << 7);
  efield |= ((fsp_state->proc_flags.hwm.sw_multiplicity & 0x1)           << 8);
...
void FSPBitField2Flags(FSPState* fsp_state, uint32_t trigger_field, uint32_t event_field)
{
  ...
  fsp_state->proc_flags.hwm.hw_multiplicity =           event_field & (0x1 << 7);
  fsp_state->proc_flags.hwm.sw_multiplicity =           event_field & (0x1 << 8);
